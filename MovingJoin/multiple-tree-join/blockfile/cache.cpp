...
Cache::Cache(int csize, int blength)
{
	...
	Level_indicator = new int[cachesize];
	   ...
	   else
	   {
		 ...
		 if (ptr == tmp)	//failed to find a free block
		 {
             ...
             for (int i = 1; i < cachesize; i++)
                if (LRU_indicator[i] > LRU_indicator[lru_index])// && Level_indicator[i] > Level_indicator[lru_index]) // && Level_indicator[i] <=3)
                    lru_index = i;
...
//----------------------------------------------------------------
bool Cache::read_block(Block block,int index, Cacheable *rt, int level)
{
	int c_ind;
...
//----------------------------------------------------------------
bool Cache::write_block(Block block, int index, Cacheable *rt, int level)
{
	int c_ind;
